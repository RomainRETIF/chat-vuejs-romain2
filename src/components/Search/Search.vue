<template>
  <div class="search-message">
    ...
    <div class="conversations">
      <div class="conversation" v-for="conv in rechercheConvs" :key="conv.id">
        <div class="author">
          <template>
            <div class="avatar"><img :src="findPicture(conv)"/></div>
            <span>{{ conv.participants.join(",") }}</span>
          </template>
        </div>
        <div class="messages" v-for="msg in 3" :key="msg.id">
          <div class="message">
...
export default {
  ...
  computed: {
    ...mapGetters(["conversations","users"]),

    rechercheConvs: function() {
      var convsfound = [];
      if(this.conversations !== null){
        this.conversations.forEach(element => {
        if(element.participants.join(",").toLowerCase().includes(this.search.toLowerCase()))
        {
          convsfound.push(element);
        }
      });
      }
    return convsfound;
